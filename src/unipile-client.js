...
export async function resolveCompanyId(companyName) {
  ...
  if (_companyCache.has(cacheKey)) {
    ...
    if (cached === null) return { success: false, error: `Company not found: ${companyName}` };
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (response.data?.id) {
        ...
        console.error(`[unipile] Company resolved: "${companyName}" → ${result.profileId}`);
        return { success: true, data: result };
      }
    } catch (err) {
      if (err.response?.status !== 404) {
        console.error(`[unipile] resolveCompanyId("${candidate}"):`, err.message);
      }
    }
  }
 ...
 */
export async function createPost(accountId, text, mediaFiles = [], mentions = []) {
  try {
    ...
    for (const media of mediaFiles) {
      ...
      if (!exists) {
        console.error(`[unipile] Skipping attachment — file not found: ${media.filePath}`);
        continue;
      }
      const size = fs.statSync(media.filePath).size;
      if (size === 0) {
        console.error(`[unipile] Skipping attachment — file is empty: ${media.filePath}`);
        continue;
      }
...
export async function deletePost(postId) {
  try {
    const response = await axios.delete(`${BASE_URL}/posts/${encodeURIComponent(postId)}`, {
      headers: authHeaders(),
      timeout: 15000
    });
