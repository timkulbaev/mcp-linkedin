...
const TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        text: {
          type: 'string',
          description: 'Post body text. Maximum 3000 characters. Include company names here if you want them @mentioned — they will be replaced with Unipile placeholders automatically.'
        },
        media: {
          type: 'array',
          items: { type: 'string' },
          description: 'Optional. Array of local file paths or URLs to attach. Supported formats: jpg, png, gif, webp, mp4. URLs are downloaded to /tmp automatically.',
          default: []
        },
        mentions: {
          type: 'array',
          items: { type: 'string' },
          description: 'Optional. Array of company names to @mention (e.g. ["Microsoft", "OpenAI"]). Each name is resolved to a LinkedIn company ID via Unipile.',
          default: []
        },
        dry_run: {
          type: 'boolean',
          description: 'DEFAULT TRUE. When true, returns a preview without publishing. Set to false only after user confirms the preview.',
          default: true
        }
      },
      required: ['text']
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        post_url: {
          type: 'string',
          description: 'LinkedIn post URL (linkedin.com/feed/update/...) or raw URN (urn:li:activity:... or urn:li:ugcPost:...)'
        },
        text: {
          type: 'string',
        ...
        dry_run: {
          type: 'boolean',
          description: 'DEFAULT TRUE. When true, returns a preview without posting. Set to false only after user confirms.',
          default: true
        }
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        post_url: {
          type: 'string',
          description: 'LinkedIn post URL (linkedin.com/feed/update/...) or raw URN (urn:li:activity:...)'
        },
        reaction_type: {
          type: 'string',
          enum: ['like', 'celebrate', 'support', 'love', 'insightful', 'funny'],
          description: 'Reaction type. One of: like, celebrate, support, love, insightful, funny. Defaults to "like".',
          default: 'like'
        }
      },
      required: ['post_url']
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        post_id: {
          type: 'string',
          description: 'The Unipile post ID returned by linkedin_publish (not the LinkedIn post URL — this is an internal Unipile identifier)'
        }
      },
      required: ['post_id']
    }
